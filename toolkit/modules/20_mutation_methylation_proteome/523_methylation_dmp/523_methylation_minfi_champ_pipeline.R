...
# ---- 补图(Bio Wingman):火山图 + Top 差异探针热图 ----
suppressWarnings(suppressMessages(library(ggplot2)))
dmp$sig <- ifelse(dmp$adj.P.Val < 0.05, ifelse(dmp$logFC > 0, "Up (case)", "Down (case)"), "NS")
.pv <- ggplot(dmp, aes(logFC, -log10(adj.P.Val), color = sig)) +
  geom_point(size = 1.2, alpha = 0.8) +
  scale_color_manual(values = c("Up (case)" = "#CC79A7", "Down (case)" = "#0072B2", "NS" = "grey80")) +
  labs(x = "logFC (case - control)", y = "-log10(adj.P)", title = "Differential methylation", color = NULL) +
...
.top <- head(rownames(dmp)[order(dmp$adj.P.Val)], 30)
if (length(.top) >= 2 && requireNamespace("pheatmap", quietly = TRUE)) {
  .ann <- data.frame(group = group); rownames(.ann) <- colnames(beta)
  png(file.path(args$outdir, "heatmap_top.png"), width = 1000, height = 900, res = 120)
  pheatmap::pheatmap(beta[.top, , drop = FALSE], annotation_col = .ann, scale = "row",
                     show_rownames = TRUE, fontsize_row = 6, main = "Top 30 differential probes")
  dev.off()
}
